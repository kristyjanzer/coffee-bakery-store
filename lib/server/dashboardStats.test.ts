...
describe("getDashboardSummary", () => {
  it("нули без деления на ноль при пустой БД", async () => {
    ...
    const { getDashboardSummary } = await import("@/lib/server/dashboardStats");
...
describe("getSalesChart", () => {
  it("days → 7 точек", async () => {
    orderFindManyMock.mockResolvedValue([]);
    const { getSalesChart } = await import("@/lib/server/dashboardStats");
...
describe("getTopProducts", () => {
  it("склеивает groupBy с именами товаров", async () => {
    ...
    const { getTopProducts } = await import("@/lib/server/dashboardStats");
...
describe("getPendingOrders", () => {
  it("возвращает до 4 заказов со статусом NEW/IN_PROGRESS", async () => {
    ...
    const { getPendingOrders } = await import("@/lib/server/dashboardStats");
